...
alm_ids <- function(doi = NULL, pmid = NULL, pmcid = NULL, wos = NULL, scp = NULL, 
  ...
  if (!is.null(doi)) doi <- doi[!grepl("image", doi)] # remove any DOIs of images
	id <- almcompact(list(doi = doi, pmid = pmid, pmcid = pmcid, wos = wos, scp = scp, 
	                      url = url, source_id = source_id, publisher_id = publisher_id))
  ...
  if (length(source_id) > 1) stop("You can only supply one source_id")
    ...
    if (info == "summary") {
      ...
      list(meta = metadf(tt), data = restmp)
        ...
        checktt <- tryCatch(tt$works[[1]], error = function(e) e)
        restmp <- if (is(checktt, "simpleError")) {
          list() 
        } else {
          getdata(x = tt$works[[1]], y = info, z = total_details, w = sum_metrics)
        }
			}
...
get_totals <- function(x, total_details=FALSE){
  ldply(x$events)
#   servs <- sapply(x$events, function(x) x$name)
#   totals <- lapply(x$events, function(x) x$metrics)
...
#   } else { 
#     ldply(totals2, function(x) as.data.frame(x)) 
#   }
}
